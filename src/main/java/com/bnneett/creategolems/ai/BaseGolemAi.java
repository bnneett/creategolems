...
public class BaseGolemAi {
    ...
    private static void initIdleActivity(Brain<BaseGolem> brain) {
        brain.addActivity(Activity.IDLE, ImmutableList.of(
            ...
            Pair.of(1, new BaseGolemAi.GolemStayNearHome(32.0D, 1.0F)),
            Pair.of(2, new RandomLookAround(UniformInt.of(150, 250), 30.0F, 0.0F, 0.0F)),
            Pair.of(3, new RunOne<BaseGolem>(ImmutableMap.of(MemoryModuleType.WALK_TARGET, MemoryStatus.VALUE_ABSENT), ImmutableList.of(
    ...
    static {
        SENSOR_TYPES = ImmutableList.of(
            ...
            SensorType.NEAREST_ITEMS, // potentially don't need for THIS golem, artifact for later
            SensorType.HURT_BY,
            SensorType.IS_IN_WATER // maybe implement losing charge faster in water? could also be dependent on golem type tho (based on material)
        );
    ...
    public static class GolemStayNearHome extends Behavior<BaseGolem> {
        ...
        public GolemStayNearHome(double maxDistance, float speed) {
            super(Map.of(), 60); //memory requirements + duration (duration???)
        ...
        protected void tick(ServerLevel level, BaseGolem golem, long gameTime) {
            ...
            if (home != null) {
                double distSq = home.distToCenterSqr(golem.getX(), golem.getY(), golem.getZ());

                if (distSq > maxDistance * maxDistance) {
                    golem.getBrain().setMemory(MemoryModuleType.WALK_TARGET, new WalkTarget(home, speed, (int)maxDistance/2)); // uh... NO idea if this works LOL
                }
            }
        }
    ...
    public static class GolemPowerDown extends Behavior<BaseGolem> {
        ...
        protected void start(ServerLevel level, BaseGolem golem, long gameTime) {
            CreateGolems.LOGGER.info("Golem {} GolemPowerDown.start, charge={}", //TODO DEBUG
        ...
        protected void stop(ServerLevel level, BaseGolem golem, long gameTime) {
            CreateGolems.LOGGER.info("Golem {} GolemPowerDown.stop state={} charge={}", //TODO DEBUG
